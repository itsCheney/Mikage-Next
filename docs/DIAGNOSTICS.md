...
`DiagnosticLogTests` 覆盖默认关闭、停用后保留、导出、JSONL 多行转义/顺序、大小轮转、超大记录、清除隔离和不可写路径。UI test 验证开关/导出/清除入口及重启持久化。完整桥接与 UIKit 编译由 iOS CI 执行；真机验收应验证开启后 A→B→A 和后台/前台时间线完整，关闭后文件不再增长。
